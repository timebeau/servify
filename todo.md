...
- 已从“架构骨架”进入“可持续开发的中早期产品”
- 当前完成度大致在 `35% ~ 45%`
- 当前最短板不是代码结构，而是：
  - 会话工作台未闭环
  - 运营数据可信度不足
  - 语音和部分运行时能力仍停留在 mock / in-memory
  - 管理后台多页仍只有展示层，没有完整运营动作
...
- [ ] Phase 1: 核心客服链路闭环
  ...
  - 任务包：
    - [ ] P1-1 接入会话消息详情查询接口与右侧消息面板
    - [ ] P1-2 接入会话发送、接管、转派、结束操作
    - [ ] P1-3 打通工单详情与会话/客服的关联展示
    - [ ] P1-4 为核心页补空态、错误态、刷新和操作反馈
    - [ ] P1-5 补主链路 e2e / smoke test
...
- [ ] Phase 2: 运营数据可信化
  ...
  - 验收标准：
    ...
    - 关键统计口径有明确定义，前后端字段一致
  - 任务包：
    - [ ] P2-1 清理 analytics 中的硬编码指标与演示值
    - [ ] P2-2 梳理统计口径文档：会话、工单、满意度、AI 使用量
    - [ ] P2-3 为 Dashboard 和 Satisfaction 接入真实图表数据
    - [ ] P2-4 为统计查询补时间范围、筛选、分页能力
    - [ ] P2-5 为关键统计能力补集成测试
...
- [ ] Phase 3: 管理后台产品化
  ...
  - 任务包：
    - [ ] P3-1 收口 Agent / Customer 页面数据与操作闭环
    - [ ] P3-2 收口 Knowledge / AI / Automation 页面真实后端能力
    - [ ] P3-3 收口 SLA / Audit / Security 页面到真实运营场景
    - [ ] P3-4 统一后台表格、详情页、操作反馈与鉴权体验
    - [ ] P3-5 补后台权限矩阵与角色验收清单
...
- [ ] 下一任务包 A：Conversation 工作台右侧消息详情与发送能力
- [ ] 下一任务包 B：Ticket 详情页关联会话与客服操作
- [ ] 下一任务包 C：Dashboard 与 Satisfaction 的真实统计图表
