...
- 当前已开始持久化内部知识文档与外部 provider `document_id` 的映射，`knowledge-docs` 删除链路也会优先使用外部 ID；但这只补齐了数据基础，不代表 Dify / WeKnora 的 provider 删除能力本身已经验收完成，因此两者目前仍不应对外声明 `deletion` capability 已完成。
