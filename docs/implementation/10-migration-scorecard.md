...
| `routing / session transfer` | `modules/routing/delivery.HandlerService` | `services.SessionTransferService` as contract impl | compatibility facade + runtime glue | `stabilized` | waiting queue 与 transfer record 已统一走 routing module；ticket assignment 同步已走 ticket module runtime adapter；主流程仍保留 session/ws 协调 |
