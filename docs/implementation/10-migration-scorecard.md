...
| `routing / session transfer` | `modules/routing/delivery.HandlerService` | `services.SessionTransferService` as contract impl | compatibility facade + runtime glue | `stabilized` | session assignment 与 system message 已走 conversation runtime adapter，waiting queue/transfer record 已走 routing module，ticket assignment 已走 ticket module runtime adapter；主流程仍保留 ws/agent load 协调 |
