...
func registerManagementRoutes(r *gin.Engine, deps Dependencies) {
	...
	handlers.RegisterCustomerRoutes(customersAPI, handlers.NewCustomerHandler(deps.CustomerService, deps.Logger))

	agentsAPI := api.Group("/")
	agentsAPI.Use(middleware.RequireResourcePermission("agents"))
	handlers.RegisterAgentRoutes(agentsAPI, handlers.NewAgentHandler(deps.AgentService, deps.Logger))
